...
const orchestrator = df.orchestrator(function* (context) {
    ...
    try {
        ...
        if (!context.df.isReplaying) telemetryClient.trackEvent({ name: "EventsFileOrchestrator", properties: { status: "call", activity: "EventsProcessUserAppointments", runId: runId, user: appointmentUser.MailAddress } });

        // wait for all the calendar fetch activities to complete.
        const result: any = yield context.df.Task.all(tasks);

        if (!context.df.isReplaying) telemetryClient.trackEvent({ name: "EventsFileOrchestrator", properties: { status: "ended", activity: "EventsProcessUserAppointments", runId: runId, user: appointmentUser.MailAddress } });


        return true;

    } catch (error) {
        telemetryClient.trackEvent({ name: "EventsFileOrchestrator", properties: { status: "exception", remarks: "error in orchestrator", error, runId } })
